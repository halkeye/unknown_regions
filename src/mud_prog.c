...
void	prog_bug		args( ( const char *str, ... ) );
/*
  ...
  *pchck = '\0';
  ...
  if ( cvar[0] == '$' )
  {
	...
	if ( !str_cmp(chck, "class" ) )
	{
		if ( IS_NPC(chkchar) )
			return -1;
		return mprog_seval((char *)ability_name[chkchar->main_ability], opr, rval, mob);
	}
  if ( !str_cmp(chck, "time" ) )
  {
	  return mprog_veval(time_info.hour, opr, atoi(rval), mob) ? TRUE : FALSE;
  }
  ...
  if ( !str_cmp(chck, "isvirgin") )
  {
      return IS_SET(chkchar->act2, EXTRA_DONE) ? TRUE : FALSE;
  }
  if ( !str_cmp(chck, "isnvirgin") )
  {
      return IS_SET(chkchar->act2, EXTRA_DONE) ? FALSE : TRUE;
  }
  if ( !str_cmp(chck, "height" ) )
  {
	  return mprog_veval(chkchar->height, opr, atoi(rval), mob) ? TRUE : FALSE;
	  /* return get_age(chkchar);*/
  }
  if ( !str_cmp(chck, "weight" ) )
  {
	  return mprog_veval(chkchar->weight, opr, atoi(rval), mob) ? TRUE : FALSE;
	  /* return get_age(chkchar);*/
  }
  if ( !str_cmp(chck, "age" ) )
  {
	  return mprog_veval(get_age(chkchar), opr, atoi(rval), mob) ? TRUE : FALSE;
	  /* return get_age(chkchar);*/
  }
  ...
  if ( chkchar )
  {
	  if ( !str_cmp( chck, "msp") )
	  {
		  if (IS_NPC(chkchar) || !chkchar->pcdata )
			  return FALSE;
		  return IS_SET(chkchar->act,PLR_SOUND) ? TRUE : FALSE;
	  }
    if ( !str_cmp(chck, "ismobinvis") )
    {
      return (IS_NPC(chkchar) && IS_SET(chkchar->act, ACT_MOBINVIS));
    }
	if ( !str_cmp(chck, "iswizinvis") )
    {
      return (!IS_NPC(chkchar) && IS_SET(chkchar->act, PLR_WIZINVIS));
    }
	...
	if (!str_cmp (chck, "wearing"))
	{
		...
		{
			if ( chkchar == obj->carried_by && obj->wear_loc > -1
				&& !str_infix(wear_locs[obj->wear_loc],rval ))
				return TRUE;
		}
		return FALSE;
	}
  ...
  if ( !str_cmp(chck, "name2") )
  {
    if ( chkchar )
      return mprog_seval(chkchar->name, opr, rval, mob) ? TRUE : FALSE;
    return mprog_seval(chkobj->name, opr, rval, mob) ? TRUE : FALSE;
  }
...
void prog_bug( const char *str, ... )
{
	...
	log_string_plus( buf, LOG_PROGBUG, LEVEL_CREATOR );
