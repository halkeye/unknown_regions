...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
	...
	else if ( xIS_SET(ch->in_room->room_flags, ROOM_HOUSE ) ) 
	{
		HOME_DATA * home;
		home = home_from_vnum(ch->in_room->vnum);
		if ( home != NULL )
			save_home2(home);
	} 
...
void do_put( CHAR_DATA *ch, char *argument )
{
    ...
    else
    {
		if ( container->item_type != ITEM_CONTAINER
			...
			&& ( container->item_type != ITEM_FURNITURE && container->value[4] != 1 ) )
		{
			send_to_char( "That's not a container.\n\r", ch );
			return;
		}
	...
	else
    {
		...
		if ( xIS_SET(ch->in_room->room_flags, ROOM_HOUSE ) && container->carried_by == NULL )
		{
			HOME_DATA * home;
			if ( (home = home_from_vnum(ch->in_room->vnum)) != NULL )
				save_home2(home);
...
void do_give( CHAR_DATA *ch, char *argument )
{
	...
	if ( victim->home && victim->owner )
	{
		if ( (get_char_world(ch, victim->owner )) != ch )
		{
			do_say(victim,"I don't want your stinking stuff");
			return;
		}
	}
	...
	if ( IS_NPC(victim) && victim->home != NULL )
		save_vendor (victim);
...
obj_ret damage_obj( OBJ_DATA *obj )
{
	...
	if ( ch && obj->item_type != ITEM_LIGHT )
...
bool remove_obj( CHAR_DATA *ch, int iWear, bool fReplace )
{
	...
	if ( obj->value[3] != WEAPON_LIGHTSABER) {
		...
		{
			act( AT_ACTION, "!!SOUND(saberoff.wav U=http://mercury.spaceports.com/~gavin1/)$n's lightsaber deactivates with a small flash and a sharp hiss.", ch, obj, NULL, TO_ROOM );
			act( AT_ACTION, "!!SOUND(saberoff.wav U=http://mercury.spaceports.com/~gavin1/)Your lightsaber deactivates with a small flash and a sharp hiss.", ch, obj, NULL, TO_CHAR );
		}
		else
		{
			act( AT_ACTION, "$n's lightsaber deactivates with a small flash and a sharp hiss.", ch, obj, NULL, TO_ROOM );
			act( AT_ACTION, "Your lightsaber deactivates with a small flash and a sharp hiss.", ch, obj, NULL, TO_CHAR );
		}
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, sh_int wear_bit )
{
    ...
    if (  1 << bit == ITEM_WIELD ||   1 << bit == ITEM_HOLD 
		...
		check_size = TRUE;
	else if ( ch->race == RACE_DUINUOGWUIN )
		check_size = TRUE;
    else if ( !IS_NPC(ch) )
	{
		switch (ch->race)
    ...
    */               
	...
	{ 
		...
		{
			act( AT_MAGIC, "It will probably be impossible to find something that fits your enormous body.", ch, NULL, NULL, TO_CHAR );
			act( AT_ACTION, "$n tries to use $p, but is too big.",
				ch, obj, NULL, TO_ROOM );
			return;
		}
	...
	case ITEM_WEAR_HANDS:
		...
		if ( xIS_SET(ch->bodyparts,BODY_L_HAND) || xIS_SET(ch->bodyparts,BODY_R_HAND) )
		{
			send_to_char( "It hurts too much!\n\r", ch);
			return;
		}
		if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
		{
				...
				if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
				{
					...
					{  
						...
						else 
						{
							if ( !IS_NPC(ch) && IS_SET( ch->act , PLR_SOUND ) )
							{
								act( AT_ACTION, "!!SOUND(saber.wav U=http://mercury.spaceports.com/~gavin1/)$n dual-wields $p.", ch, obj, NULL, TO_ROOM );
								act( AT_ACTION, "!!SOUND(saber.wav U=http://mercury.spaceports.com/~gavin1/)You dual-wield $p.", ch, obj, NULL, TO_CHAR );
							}
							else
							{
								act( AT_ACTION, "$n dual-wields $p.", ch, obj, NULL, TO_ROOM );
		...
		if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
		{
			...
			{  
				...
				else 
				{
					if ( !IS_NPC(ch) && IS_SET( ch->act , PLR_SOUND ) )
					{
						act( AT_ACTION, "!!SOUND(saber.wav U=http://mercury.spaceports.com/~gavin1/)$n wields $p.", ch, obj, NULL, TO_ROOM );
						act( AT_ACTION, "!!SOUND(saber.wav U=http://mercury.spaceports.com/~gavin1/)You wield $p.", ch, obj, NULL, TO_CHAR );
					}
					else
					{
						act( AT_ACTION, "$n wields $p.", ch, obj, NULL, TO_ROOM );
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "Gavin gives you his thanks for your sacrifice.\n\r",ch );
    act( AT_ACTION, "$n sacrifices $p to Gavin.", ch, obj, NULL, TO_ROOM );
...
void do_auction (CHAR_DATA *ch, char *argument)
{
	...
	if ( !has_comlink(ch) )
	{
		send_to_char( "You need a Comlink to participate in the auctions!\n\r", ch);
		return;
	}
    ...
    if (arg1[0] == '\0')
    {
		...
		sprintf( buf, "&RHere is the info on the Item currently Being Actioned:\n\rName: &B'%s'&R Type: &B%s&R.\n\rIt normally costs &B%d&R.\n\r",
		obj->name, aoran( item_type_name( obj ) ), obj->cost );
	    send_to_char( buf, ch );   
		sprintf( buf, "&RWorn on: &B%s&R\n\r", 
	    ...
	    {
		case ITEM_ARMOR:
			ch_printf( ch, "&RCurrent armor level is is &B%d&R. ( based on current condition )\n\r", obj->value[0] );
			ch_printf( ch, "&RMaximum armor level is &B%d&R. ( based on top condition )\n\r", obj->value[1] );
			break;
		}
         
	    if ( ( obj->item_type == ITEM_CONTAINER ) && ( obj->first_content ) )
	    {
		send_to_char( "&GContents:&G&W\n\r", ch );
	    ...
	    if (IS_IMMORTAL(ch))
	    {
		sprintf(buf, "&RSeller: &B%s&R.  &RBidder: &B%s&R.  Round: &B%d&R.\n\r",
                        auction->seller->name, auction->buyer->name,
                        (auction->going + 1));
		send_to_char(buf, ch);
		sprintf(buf, "&RTime left in round: &B%d&R.\n\r", auction->pulse);
		send_to_char(buf, ch);
	    }
            return;
	}
	...
	/*	    ch_printf( ch, "Bid: %d\n\r",newbet);	*/
            ...
            */
/*			if ( auction->item->cost ) {
				if ( auction->item->cost < 1000 )
					bet = 100;
				if ( auction->item->cost < 100000 )
					bet = 10000;
				if ( auction->item->cost < 1000000 )
					bet = 100000;
			}
			else */
				bet = 100;
            ...
            if (newbet < (auction->bet + bet))
            {
                sprintf (buf,"You must at least bid %d credits over the current bid.\n\r", bet );
				send_to_char (buf,ch);
                return;
            }
