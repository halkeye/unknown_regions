...
void do_sset( CHAR_DATA *ch, char *argument )
{
	...
	if ( get_trust(ch) > LEVEL_GREATER )
	{
	  ...
	  send_to_char( "  type damtype acttype classtype powertype flag dice value affect\n\r", ch );
	  send_to_char( "  rmaffect level hit miss die imm (char/vict/room)\n\r", ch );
	  send_to_char( "  teachers \n\r",			ch );
	...
	if ( !str_cmp( arg2, "prototype" ) )
	{
		...
		if ( get_trust(ch) <= LEVEL_GREATER ) /* Level 61 Or Higher */
		{
...
{
	send_to_char("This has been removed - Gavin\n\r.",ch);
	return;
}
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    percent  = number_percent( ) + ( IS_AWAKE(victim) ? 10 : -50 )
	       ...
	       + (get_curr_dex(ch) - 10) + times_killed( ch, victim )*7;

    if ( victim->position == POS_FIGHTING
    ||   percent > ( IS_NPC(ch) ? 90 : (ch->pcdata->learned[gsn_steal] + get_curr_dex(ch)) ) ) 
    {
	/*
	 * Failure.
	 */
...
void do_punch( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC(ch) || number_percent( ) < ch->pcdata->learned[gsn_punch] )
    {
		...
		{
			if ( !IS_NPC(victim) && IS_SET(victim->pcdata->cyber, CYBER_CHEST) )
			{
				if ( !xIS_SET(ch->bodyparts,BODY_L_HAND) )
					xSET_BIT(ch->bodyparts, BODY_L_HAND);
				else if ( !xIS_SET(ch->bodyparts,BODY_R_HAND ) )
					xSET_BIT(ch->bodyparts, BODY_R_HAND);
			}
			else
			{
				if ( !xIS_SET(victim->bodyparts,BODY_RIBS) )
					xSET_BIT(victim->bodyparts, BODY_RIBS);
			}
		}
		else if ( number_percent() <= 25 && !IS_IMMORTAL(ch) )
		{
			if ( !xIS_SET(victim->bodyparts,BODY_JAW) )
				xSET_BIT(victim->bodyparts, BODY_JAW);
		}
...
void do_bash( CHAR_DATA *ch, char *argument )
{
   ...
   chance = (((get_curr_dex(victim) + get_curr_str(victim))
    ...
    || (number_percent( ) + chance) < ch->pcdata->learned[gsn_bash] )
    {
		...
		if (( number_percent() >= 25 || ( !IS_NPC(victim) && IS_SET(victim->pcdata->cyber, CYBER_CHEST) ) ) && !IS_IMMORTAL(ch) )
		{
			if ( !xIS_SET(ch->bodyparts,BODY_L_HAND) )
				xSET_BIT(ch->bodyparts, BODY_L_HAND);
			else if ( !xIS_SET(ch->bodyparts,BODY_R_HAND ) )
			 ...
			 if ( ship->alarm > 0 )
			 {
				 ...
				 if ( str_cmp("Unowned", ship->owner) && str_cmp("Public", ship->owner) )
				 {
					 if ( (clan = get_clan(ship->owner) ) == NULL ) {
						 ...
						 if (owner != NULL) 
							 ch_printf( owner, "&W[&R^zALARM&W^x]&R %s attempting to pick %s.\n\r", ch->name, ship->name  );
						 if ( ship->pilot)
							 pilot1 = get_char_world(ch,ship->pilot);
						 if (pilot1 != NULL) 
							 ch_printf( pilot1, "&W[&R^zALARM&W^x]&R %s attempting to pick %s.\n\r", ch->name, ship->name  );
						 if ( ship->copilot)
							 pilot2 = get_char_world(ch,ship->copilot);
						 if (pilot2 != NULL) 
							 ch_printf( pilot2, "&W[&R^zALARM&W^x]&R %s attempting to pick %s.\n\r", ch->name, ship->name  );
					 }
					 else 
					 {
						 for( d = first_descriptor; d; d = d->next )
						 {
							 ...
							 if( d->connected == CON_PLAYING )
							 {
								 ...
								 {
									 ch_printf( vch, "&W[&R^zALARM&W^x]&R %s attempting to pick %s.\n\r", ch->name, ship->name  );
									 continue;
								 }
							 }
						 }
				 ...
				 sprintf( buf, "&W[&R^zALARM&W^x]&R %s attempting to pick %s.", ch->name, ship->name  );
...
void do_aid( CHAR_DATA *ch, char *argument )
{
	...
	
    {
         ...
         xpgain = UMIN( number_range(1,100)*50 ,( exp_level(ch->skill_level[MEDIC_ABILITY]+1) - exp_level(ch->skill_level[MEDIC_ABILITY]) ) );
...
void do_listen (CHAR_DATA *ch, char *argument)
{
  ...
  if (ch->mount) 
  {
	send_to_char("Your mount is making to much noise for you to listen.\n\r", ch);
	return;
  }
  ...
  {
	send_to_char("You have trouble hearing underwater.\n\r", ch);
	return;
  }

  one_argument (argument, arg);
/*  WAIT_STATE (ch, SkillTable.GetBeats (gsn_listen)); */
    WAIT_STATE (ch, PULSE_VIOLENCE);
  ...
  if (arg[0] == '\0')  /*No argument means the character is listening in the room he/she is in*/
  {
	for (tch = ch->in_room->first_person; tch; tch = tch_next) 
	{
		  ...
		  if (ch != tch
			  ...
			  && !IS_AFFECTED(tch, AFF_SNEAK))
				count++;
  	}

	act (AT_GREY, "\nListening...", ch, NULL, NULL, TO_CHAR);
	set_char_color (AT_SKILL, ch);

	if (number_percent() < ch->pcdata->learned[gsn_listen])
	{
  		if (count > 0) 
		{
			if (ch->top_level > 15)
			{
			  sprintf(buf, "You hear what might be %d %s lurking about.\n\r", count, ((count == 1)? "creature": "creatures"));
			  send_to_char( buf, ch );
			}
			else
			{
			  sprintf( buf, "You hear the presence of %s.\n\r",((count == 1)? "an unseen creature": "unseen creatures"));
	      	  send_to_char( buf, ch );
			}
           		learn_from_success (ch, gsn_listen);
	...
	{
	  send_to_char("You don't hear anything unusual.\n\r", ch);
	  learn_from_failure (ch, gsn_listen);
	}

  	act (AT_ACTION, "$n seems to listen intently for something.", ch, NULL, NULL, TO_ROOM);
  	return;
  }

  else 
  {
	if ((dir = get_door(argument)) == -1) /* The argument must be a direction*/
	{  
	...
	if ((pexit = get_exit(ch->in_room, dir)) == NULL)
	{
	  send_to_char("You don't hear anything from that direction.\n\r", ch);
	  return;
	}

	act (AT_GREY, "\nListening $t...", ch, dir_name[dir], NULL, TO_CHAR);
	set_char_color (AT_SKILL, ch);
	...
	char_to_room (ch, pexit->to_room);
		...
		{
			tch_next = tch->next_in_room;
			if (tch->top_level < LEVEL_IMMORTAL
				&& !can_see (ch, tch)
				&& !IS_AFFECTED( tch, AFF_SNEAK ))
				count++;
		}
	...
	if (number_percent() < ch->pcdata->learned[gsn_listen])
	{
		if (count > 0) 
		{
			if (ch->top_level > 15) /*arbitrary, adjust to suit*/
			{
			  sprintf( buf, "You hear what sounds like %d %s lurking %s%s.\n\r", count,
					    ((count == 1)? "creature": "creatures"),
					    ((dir == 5)? "below": (dir == 4)? "above": "to the "),
					    ((dir == 5)? "": (dir == 4)? "": dir_name[dir]));
			send_to_char( buf, ch );
			}
			else
			{
			  sprintf( buf, "You hear an odd rustling%s%s.\n\r",
					    ((dir == 5)? "from below": (dir == 4)? "from above": "to the "),
					    ((dir == 5)? "": (dir == 4)? "": dir_name[dir]));
			send_to_char( buf, ch );
			}
			learn_from_success (ch, gsn_listen);
...
void do_butcher(CHAR_DATA *ch, char *argument)
{
	...
	if ( ( pObjIndex = get_obj_index( OBJ_VNUM_STEAK ) ) == NULL )
	{
	...
	buf[0]='\0';
	strcat(buf,"A ");
	strcat(buf,str_dup(race_table[obj->value[4]]->race_name));
	strcat(buf," steak is here.");
	steak = create_object( get_obj_index( OBJ_VNUM_STEAK ), 0 );
	steak->description = str_alloc(buf);
	steak->value[0] = 75;
	steak->value[1] = 100;
	buf[0]='\0';
	strcat(buf,"A ");
	strcat(buf,str_dup(race_table[obj->value[4]]->race_name));
	strcat(buf," steak.");
	steak->short_descr=str_alloc(buf);
	...
	if(number_percent( ) < ch->pcdata->learned[gsn_butcher])
	{
		...
		case 1:
			...
			act(AT_ACTION, "$n butchers a corpse and creates a steak.\n\r", ch, NULL, NULL, TO_ROOM );
			act(AT_ACTION, "You butcher a corpse and create a steak.\n\r",  ch, NULL, NULL, TO_CHAR );
			break;
		case 2:
			steak = create_object( get_obj_index( OBJ_VNUM_STEAK ), 0 );
			obj_to_room( steak, ch->in_room );
			steak = create_object( get_obj_index( OBJ_VNUM_STEAK ), 0 );
			obj_to_room( steak, ch->in_room );
			act(AT_ACTION, "$n butchers a corpse and creates two steaks.\n\r", ch, NULL, NULL, TO_ROOM );
			act(AT_ACTION, "You butcher a corpse and create two steaks.\n\r",  ch, NULL, NULL, TO_CHAR );
			break;
		case 3:
			steak = create_object( get_obj_index( OBJ_VNUM_STEAK ), 0 );
			obj_to_room( steak, ch->in_room );
			...
			steak = create_object( get_obj_index( OBJ_VNUM_STEAK ), 0 );
			obj_to_room( steak, ch->in_room );
			act(AT_ACTION, "$n butchers a corpse and creates three steaks.\n\r", ch, NULL, NULL, TO_ROOM );
			act(AT_ACTION, "You butcher a corpse and create three steaks.\n\r",  ch, NULL, NULL, TO_CHAR );
			break;
		case 4:
			steak = create_object( get_obj_index( OBJ_VNUM_STEAK ), 0 );
			obj_to_room( steak, ch->in_room );
			...
			steak = create_object( get_obj_index( OBJ_VNUM_STEAK ), 0 );
			obj_to_room( steak, ch->in_room );
			act(AT_ACTION, "$n butchers a corpse and creates four steaks.\n\r", ch, NULL, NULL, TO_ROOM );
			act(AT_ACTION, "You butcher a corpse and create four steaks.\n\r",  ch, NULL, NULL, TO_CHAR );
			break;
		}
		
		learn_from_success(ch , gsn_butcher );
	}
	else
	{
		act(AT_ACTION, "$n fails to butcher a corpse, and destroys it.\n\r", ch, NULL, NULL, TO_ROOM );
		act(AT_ACTION, "You fail to butcher a corpse, and destroy it.\n\r",   ch, NULL, NULL, TO_CHAR );
		learn_from_failure( ch , gsn_butcher );
	}
	/* dump items caried */
