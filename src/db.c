...
void boot_db( bool fCopyOver )
{
	...
	*/
	...
	ASSIGN_GSN( gsn_makekeyring  , "makekeyring" );
	...
	ASSIGN_GSN( gsn_makedroid  , "makedroid" );        
	...
	ASSIGN_GSN( gsn_vandalize,	"vandalize" );
	...
	ASSIGN_GSN( gsn_askani,		"askani" );
	
	ASSIGN_GSN( gsn_addpatrol,     "add_patrol" );
	ASSIGN_GSN( gsn_butcher,		"butcher" );
	ASSIGN_GSN( gsn_listen, 		"listen" );
	ASSIGN_GSN( gsn_bomb, 		"planetary bombing" );
	ASSIGN_GSN( gsn_evasive,		"evasive" );
	ASSIGN_GSN( gsn_birth,				"birth" );
	ASSIGN_GSN( gsn_abort,				"abort" );
	ASSIGN_GSN( gsn_stealcode,		"obtaincode" );
	ASSIGN_GSN( gsn_steal,		"steal" );
	ASSIGN_GSN( gsn_cosmetic,		"cosmetic" );
	ASSIGN_GSN( gsn_diagnose, "diagnose" );
	ASSIGN_GSN( gsn_autopsy, "autopsy");
	ASSIGN_GSN( gsn_ship_masking, "ship_masking" );
	ASSIGN_GSN( gsn_washup, "washup" );
	ASSIGN_GSN( gsn_make_infect_med, "make_infect_med" );
	ASSIGN_GSN( gsn_inject, "inject" );
    }
    ...
    */
	...
	if( fCopyOver )
        {
          log_string("Copyover: Running copyover recover...");
          copyover_recover();
        }		
		...
		load_slays( );
#ifdef RESTORE_ADDON
		log_string( "Loading Custom Restores" ); /* Custom Restores - Gavin 5-26-2000 */
		load_restores( );
#endif
...
void free_char( CHAR_DATA *ch )
{
    ...
    if ( ch->pcdata && !IS_NPC(ch) )
    {
       ...
       if ( IS_SET(ch->act2, EXTRA_PREGNANT)
                && ( !(ch->pcdata->pregnacy.hour > -1) || !(ch->pcdata->pregnacy.day > -1)
                || !(ch->pcdata->pregnacy.month > -1 )|| !( ch->pcdata->pregnacy.year > -1 ) ) )
        {
                ch->pcdata->pregnacy = time_info;
        }
        if ( !(ch->pcdata->birthday.year > -1) || !(ch->pcdata->birthday.day > -1) || !(ch->pcdata->birthday.month > -1 ) || !(ch->pcdata->birthday.hour > -1 ))
                ch->pcdata->birthday = time_info;  /* Added for player birthdays - Voltec 98*/
...
void log_string_plus( const char *str, sh_int log_type, sh_int level )
{
	...
	default:
		...
		break;
	case LOG_PROGBUG:
		to_channel( str + offset, CHANNEL_PROGBUG, "", level );
		break;
	case LOG_BUILD:
	...
	case LOG_COMM:
		...
		break;
/* 	case LOG_AUTH:
		to_channel( str + offset, CHANNEL_AUTH, "Auth", level );
		break;*/
...
void make_wizlist( )
{
  ...
  buf[0] = '\0';
  unlink( WIZLIST_FILE );
  towizfile( " Masters of Unknown Regions " );
  ilevel = 65535;
  for ( wiz = first_wiz; wiz; wiz = wiz->next )
  {
   if ( wiz->level > LEVEL_AVATAR )
   {
	...
	buf[0] = '\0';
      ...
      switch(ilevel)
      {
	case MAX_LEVEL -  0: towizfile( "     &RHead Admin&W" );	break;
	case MAX_LEVEL -  1: towizfile( "     &RThe Masters&W" );		break;
	case MAX_LEVEL -  2: towizfile( "     &RThe Creator&W" );	break;
	case MAX_LEVEL -  3: towizfile( "     &RThe Council&W" );		break; 
	case MAX_LEVEL -  4: towizfile( "     &RExalted Ones&W" );	break;
	case MAX_LEVEL -  5: towizfile( "     &RAscendant Ones&W" );	break;
	case MAX_LEVEL -  6: towizfile( "     &RGreater Ones&W" );	break;
	case MAX_LEVEL -  7: towizfile( "     &RRegular Ones&W" );		break;
	case MAX_LEVEL -  8: towizfile( "     &RLesser Ones&W" );	break;
	case MAX_LEVEL -  9: towizfile( "     &RImmortals&W" );		break;
	case MAX_LEVEL - 10: towizfile( "     &RDemi Gods&W" );		break;
	case MAX_LEVEL - 11: towizfile( "     &RSaviors&W" );		break;
	case MAX_LEVEL - 12: towizfile( "     &RCreators&W" );		break;
	case MAX_LEVEL - 13: towizfile( "     &RAcolytes&W" );		break;
	case MAX_LEVEL - 14: towizfile( "     &RRetired&W" );		break;
	case MAX_LEVEL - 15: towizfile( "     &RRetired&W" );		break;
	case MAX_LEVEL - 16: towizfile( "     &RGuests&W" );		break;
	default:             towizfile( "     &RBuilders&W" );	break;
      }
    }
