...
void room_explode_1( OBJ_DATA *obj , CHAR_DATA *xch, ROOM_INDEX_DATA *room , int blast )
{
          ...
          {        
			  ...
			  if ( robj != obj && robj->item_type != ITEM_SCRAPS 
				  && robj->item_type != ITEM_CORPSE_NPC && robj->item_type != ITEM_CORPSE_PC && robj->item_type != ITEM_DROID_CORPSE)
				  make_scraps( robj );        
		  }
		if ( xIS_SET(room->room_flags, ROOM_SPACECRAFT) )
		{
		    SHIP_DATA *ship;
			for ( ship = first_ship; ship; ship = ship->next )
			{
				if ( room->vnum >= ship->firstroom && room->vnum <= ship->lastroom )
				{
					sprintf( buf, "The explosion from %s's grenade rocks the ship.",xch->name );
					echo_to_ship( AT_RED , ship , buf );
					sprintf( buf, "GRENADE: %s's grenade hits ship called %s", xch->name, ship->name );
					log_string(buf);
					damage_ship(ship,0,dam/10);
				}
			}
		}
		{
...
/* spell cast upon wear/removal of an object	-Thoric */
	...
	&&   skill->type == SKILL_SPELL )
	   if ( (retcode=(*skill->spell_fun) ( mod, 1/*ch->skill_level[FORCE_ABILITY]*/, ch, ch )) == rCHAR_DIED
	   ||   char_died(ch) )
	      return;
	break;
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
	...
	if ( (xIS_SET(ch->bodyparts,BODY_L_HAND) || xIS_SET(ch->bodyparts,BODY_R_HAND))
		&& ( iWear == WEAR_HANDS || iWear == WEAR_DUAL_WIELD || iWear == WEAR_HOLD || iWear == WEAR_WIELD ) )
	{
   ...
   for ( RQueue = first_relation; RQueue; RQueue = rq_next ) {
	...
	for ( wch = first_char; wch; wch = wch->next )
	{
		...
		if ( IS_SET( ch->act, ACT_PET ) )
			REMOVE_BIT( wch->act, PLR_BOUGHT_PET );
	    }
	}
    }
...
bool room_is_private( CHAR_DATA *ch , ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if ( (xIS_SET(pRoomIndex->room_flags, ROOM_PLR_HOME) 
		&& ch->pcdata->plr_home != pRoomIndex) )
        return TRUE; 

	if ( xIS_SET(pRoomIndex->room_flags, ROOM_HOUSE) )
	{
		HOME_DATA * home;
		home = home_from_entrance(pRoomIndex->vnum);
		if ( home && !check_member(ch,home) && home->class <= APARTMENT_HOME )
		return TRUE; 
	}
...
char *affect_loc_name( int location )
{
	...
	case APPLY_CONTRACEPTIVE: return "contraceptive";
	default: return "Error, contact coder";
...
char *cyber_bit_name(int cyber_flags)
{
	...
	if (cyber_flags & CYBER_REACTOR  	) strcat(buf, " Internal Reactor\n\r");
...
OBJ_DATA *group_object( OBJ_DATA *obj1, OBJ_DATA *obj2 )
{
	...
	if ( obj1->pIndexData == NULL || obj2->pIndexData == NULL)
 ...
 * Fireblade
 */
AREA_DATA *get_area_by_filename(char *name)
{
	AREA_DATA *pArea;
	...
	for(pArea = first_area; pArea; pArea = pArea->next)
	{
		if( !str_cmp(name, pArea->filename) )
			break;
	}
