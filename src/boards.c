...
bool is_note_to( CHAR_DATA *ch, NOTE_DATA *pnote )
{
	...
	if ( IS_IMMORTAL(ch) && is_name( "imms", pnote->to_list ) )
		return TRUE;
	if ( !IS_IMMORTAL(ch) && is_name( "mortals", pnote->to_list ) )
		return TRUE;
...
void do_note( CHAR_DATA *ch, char *arg_passed, bool IS_MAIL )
{
		...
		if (!IS_MAIL)
		{
			...
			for ( pnote = board->first_note; pnote; pnote = pnote->next )
			{
				...
				if ( (first_list && vnum >= first_list) || !first_list )
					...
					vnum, is_note_to( ch, pnote ) ? ')' : '}', pnote->sender,
					(pnote->voting != VOTE_NONE) ? (pnote->voting == VOTE_OPEN ? 'V' : 'C') : ':',
		...
		if (!IS_MAIL)
		{
			...
			for ( pnote = board->first_note; pnote; pnote = pnote->next )
			{
				...
				if ( vnum == anum || fAll )
				{
					wasfound = TRUE;
					pager_printf(ch, "%s\n\r" "[%3d] %s: %s\n\r" "To: %s\n\r" "%s",
						pnote->date, vnum, pnote->sender, pnote->subject,
	...
	if ( !str_cmp( arg, "vote" ) )
	{
		...
		{
			if ( str_cmp( ch->name, pnote->sender ) && ( get_trust(ch) != MAX_LEVEL ) )
			{
	...
	if ( !str_cmp( arg, "to" ) )
	{
		...
		if ( IS_MAIL )
		{
			if ( !str_cmp( arg_passed, "all" ) || !str_cmp( arg_passed, "mortals" ) 
				|| !str_cmp( arg_passed, "imms" ) || stat( fname, &fst ) != -1 ) 
			{
				paper->value[2] = 1;
...
void do_bstat( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(
		...
		"&P|&W Filename     &P|&W Vnum   &P|&W Read &P|&W Post &P|&W Rmv  &P|&W Max  &P|&W Posts &P|&W Type &P|\n\r"
		"&P+&B--------------&P+&B--------&P+&B------&P+&B------&P+&B------&P+&B------&P+&B-------&P+&B------&P+\n\r",ch);
    ch_printf( ch, "&P|&W %-12s &P|&W %6d &P|&W %4d &P|&W %4d &P|&W %4d &P|&W %4d &P|&W %5d &P|&W %5d |\n\r",
	...
	send_to_char(
		"&P+&B------------------&P+&B------------------&P+&B------------------&P+\n\r"
		"&P|&W Read_group       &P|&W Post_group       &P|&W Extra_readers    &P|\n\r"
		"&P+&B------------------&P+&B------------------&P+&B------------------&P+\n\r",ch);
    ch_printf( ch, "&P|&W %-15s &P|&W %-15s &P|&W %-15s &P|\n\r",
