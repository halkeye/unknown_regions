...
CHAR_DATA *get_char_room_see( CHAR_DATA *ch, char *argument );
...
ch_ret damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
     ...
     * Update by ackbar to ignore damaging equipment and eq bonuses
     * if your in an arena room.
     */
    if (dam > 10 && dt != TYPE_UNDEFINED && !xIS_SET( ch->in_room->room_flags, ROOM_ARENA))
    {
	...
	victim->hit = 1;
	if (xIS_SET( victim->in_room->room_flags, ROOM_NODEATH ) && victim->hit < 1 && !IS_NPC(victim) )
	{
		stop_fighting(ch, TRUE);
		stop_fighting(victim, TRUE );
		victim->hit = 50;
    ...
    if (xIS_SET( ch->in_room->room_flags, ROOM_ARENA ) && victim->hit < 1 && !IS_NPC(victim) )
    {
      	stop_fighting( ch, TRUE );
      	stop_fighting( victim, TRUE );
        victim->hit = victim->max_hit;
     	...
     	update_pos(ch);
		update_pos (victim);
     	send_to_char("A Godly Glow Surrounds your body and life is returned to you.\n\r", victim);
     	sprintf( buf, "You have been defeated in combat by %s\n\r", ch->name);
     	send_to_char(buf, victim);
		act(AT_RED, "$n has defeated $N in combat",ch, NULL, victim,TO_ROOM);
		return rNONE;
      }
...
void set_fighting( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	{
		CHAR_DATA * carry;
		carry = ch->char_carrying;
		clear_carry(carry);
		clear_carry(ch);
		carry->position = POS_STANDING;
		ch->position = POS_STANDING;
		act( AT_ACTION, "You quickly drop $N and prepare to fight.", ch, NULL, carry, TO_CHAR );
		act( AT_ACTION, "$n drops you and starts fighting.", ch, NULL, carry, TO_VICT );
		act( AT_ACTION, "$n quickly drops $N and starts fighting.", ch, NULL, carry, TO_NOTVICT );
	}

	if ( victim->char_carrying != NULL )
	{
		CHAR_DATA * carry;
		carry = victim->char_carrying;
		clear_carry(carry);
		clear_carry(victim);
		carry->position = POS_STANDING;
		victim->position = POS_STANDING;
		act( AT_ACTION, "You quickly drop $N and prepare to fight.", victim, NULL, carry, TO_CHAR );
		act( AT_ACTION, "$n drops you and starts fighting.", victim, NULL, carry, TO_VICT );
		act( AT_ACTION, "$n quickly drops $N and starts fighting.", victim, NULL, carry, TO_NOTVICT );
	}

	if ( ch->char_carriedby != NULL )
	{
		CHAR_DATA * carry;
		carry = ch->char_carriedby;
		clear_carry(carry);
		clear_carry(ch);
		carry->position = POS_STANDING;
		ch->position = POS_STANDING;
		act( AT_ACTION, "$N quickly drops you as you start to fight.", ch, NULL, carry, TO_CHAR );
		act( AT_ACTION, "You quickly drop $n as $n starts to fight.", ch, NULL, carry, TO_VICT );
		act( AT_ACTION, "$N quickly drops $n as $n begins to fight.", ch, NULL, carry, TO_NOTVICT );
	}

	if ( victim->char_carriedby != NULL )
	{
		CHAR_DATA * carry;
		carry = victim->char_carriedby;
		clear_carry(carry);
		clear_carry(victim);
		carry->position = POS_STANDING;
		ch->position = POS_STANDING;
		act( AT_ACTION, "$N quickly drops you as you start to fight.", victim, NULL, carry, TO_CHAR );
		act( AT_ACTION, "You quickly drop $n as $n starts to fight.", victim, NULL, carry, TO_VICT );
		act( AT_ACTION, "$N quickly drops $n as $n begins to fight.", victim, NULL, carry, TO_NOTVICT );
	}
...
void raw_kill( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( (ch != victim ) && ( NEW_AUTH( victim ) && !IS_IMMORTAL(ch) ) ) /* new auth */
    {
      bug( "raw_kill: killing unauthed", 0 );
      return;
    }
	if (xIS_SET( ch->in_room->room_flags, ROOM_NODEATH ) && victim->hit < 1 && !IS_NPC(victim) )
	{
		stop_fighting(ch, TRUE);
		stop_fighting( victim, TRUE );
		victim->hit = 50;
	...
	if ( victim->in_room && IS_NPC(victim) && victim->in_room->area && victim->in_room->area->planet )
	{
		...
		/*if ( victim->in_room && IS_NPC(victim) && IS_SET( victim->act, ACT_CITIZEN ) && victim->in_room->area && victim->in_room->area->planet )*/
		{
...
/*
 * Unknown Regions Change Follows 
 */

    for ( home = first_home; home; home = home->next )
    {
         if ( !str_cmp( home->owner, victim->name ) )
              resethome( home);           
    }

    if ( victim->pcdata->plr_home )
    {
      ROOM_INDEX_DATA *room = victim->pcdata->plr_home;
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( !IS_NPC(gch) && IS_NPC(victim) && gch->pcdata && gch->pcdata->clan && victim->mob_clan
	&& !str_cmp( gch->pcdata->clan->name , victim->mob_clan->name ) )
	{
	     xp = 0;
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
   ...
   /*		     10 * percent					*/
    ...
    {    
         ...
         sprintf( sound , "!!SOUND(blaster V=%d  U=http://mercury.spaceports.com/~gavin1/)" , vol );
...
void do_kill( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( victim = get_char_room_see( ch, arg ) ) == NULL )
    {
...
void do_murder( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_NPC(victim) )
	{
		send_to_char("You Must Kill A Mob.",ch);
		return;
	}
	
	if ( IS_NPC(ch) )
	{
		do_say(ch, "I cannot do that sorry.");
		return;
	}
...
void do_trip( CHAR_DATA *ch, char *argument )
{
    ...
    {
		act( AT_HIT, "You pull $N's legs from underneath $M.", ch, NULL, victim, TO_CHAR);
		act( AT_HITME, "$n pulls your legs from underneath you.", ch, NULL, victim, TO_VICT );
		act( AT_ACTION, "$n pulls $N's legs from underneath $M.", ch, NULL, victim, TO_NOTVICT );
		learn_from_success( ch, gsn_trip );
		global_retcode = damage( ch, victim, number_range( 1, ch->skill_level[COMBAT_ABILITY] ), gsn_trip );
    }
    else
    {	
		act( AT_HIT, "You swing your foot but connect with nothing.", ch, NULL, victim, TO_CHAR);
		act( AT_HITME, "$n swing $s foot but connect with nothing.", ch, NULL, NULL, TO_ROOM );
	...
	{
		if ( !IS_NPC(victim) && !IS_SET( victim->pcdata->cyber, CYBER_LEGS ) )
		{
			if ( !xIS_SET(victim->bodyparts,BODY_L_LEG ) )
			{
				act( AT_HITME, "You hit $N's legs!", ch, NULL, victim, TO_CHAR);
				act( AT_HITME, "Ouch!", ch, NULL, victim, TO_VICT);
				xSET_BIT(victim->bodyparts, BODY_L_LEG);
			}
			else if ( !xIS_SET(victim->bodyparts,BODY_R_LEG) )
			{
				act( AT_HITME, "You hit $N's legs!", ch, NULL, victim, TO_CHAR);
				act( AT_HITME, "Ouch!", ch, NULL, victim, TO_VICT);
				xSET_BIT(victim->bodyparts, BODY_R_LEG);
			}
		}
		else if ( ( !IS_NPC(victim) && IS_SET(victim->pcdata->cyber, CYBER_LEGS) ) && ( !IS_NPC(ch) && !IS_SET(ch->pcdata->cyber, CYBER_LEGS) ) )
		{
			if ( !xIS_SET(ch->bodyparts,BODY_L_LEG ) )
			{
				act( AT_HITME, "Ouch! that hurt!", ch, NULL, NULL, TO_CHAR);
				act( AT_HITME, "You Hit $N's legs fiarly hard", ch, NULL, victim, TO_VICT);
...
CHAR_DATA *get_char_room_see( CHAR_DATA *ch, char *argument )
{
	...
	if ( (can_see( ch, rch ) || rch->race == RACE_DEFEL )
